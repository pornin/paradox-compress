 ...
 *   1. If len(data) <= 16 + 2*Nlen:
 *          Return data
 *   1. Compute d1 = DEFLATE-compress(data)
 *   2. If len(d1) < len(data) - 32 - 2*Nlen:
 *          Set T = 0
 *          Return d1 || T || VDF-Eval(T, d1)
 *   3. Split data = d2 || T || f || pi with T a counter (16 bytes), and
 *      f and pi two sequences of Nlen bytes.
 *   4. If f and pi can be decoded as elements of G (in the 1..(N-1)/2 range)
 *      and VDF-Verify(T, d2, f, pi) == true:
 *          Return d2 || T+1 || VDF-Eval(T+1, d2)
 *   5. Return data
 *
...
public class ParadoxCompress {
	 ...
	 */
	static ZInt DEFAULT_MODULUS = ZInt.Parse("0xC98F9A7148778F4D5C1F741DE4" +
        "DA3033ACE4B962513D2A5FC04320B0B2BAC39B205551812091E5C82DA46F83D77" +
        "C32B329F874EF0D71945E179DFF0D25A3251B48D005451BB370263B9A2254CD7B" +
        "E02785305B8247A0153B5FCF012A66F394916E97D9195DCF5FD753980BB64AA07" +
        "9ED08931DB4C24DE008E0A93E6482F5BCA0E8438FB9BAD409F5882FD8AD28D149" +
        "F2468B62758303C71D07754EAC37C570D785AE659A79A26769EB524D839FDACBC" +
        "DB39DD3BD7DEA24F51E0CE2F1DA441EC7CBB5724C728CBE76898109C6179EE44A" +
        "E959B67039EE2D738E8E525B2EDD7F76115C440472CAF8467DC63C31659C7FB8D" +
        "D4A46122E6D0870E7C9B8ADEAE3F62D");
	...
	public ParadoxCompress(ZInt N)
	{
		...
		if (N.Sign <= 0 || (8 * Nlen) < 1024 || !N.TestBit(0)) {
			// We reject a few obviously wrong moduli:
	...
	{
		// Elements are normalized to 1..(N-1)/2 when encoding.
		if (x > hN) {
			x = N - x;
		}
	...
	ZInt HashToSmallPrime(ZInt g, ZInt T, ZInt f)
	{
		...
		// We look for the next prime in a rather inefficient
		// way; we could do some sieving to skip most bad
		// candidates. But it's good enough for a demo.
		if (ell <= 2) {
			ell = 3;
		} else {
			if (!ell.TestBit(0)) {
